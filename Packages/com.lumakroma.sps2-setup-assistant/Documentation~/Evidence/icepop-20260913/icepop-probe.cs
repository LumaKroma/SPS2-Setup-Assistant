...
try {
 ...
 var iceRoot=root.testPlug;
 if(iceRoot.transform.localScale!=UnityEngine.Vector3.one*.7f)throw new System.Exception("IcePop scale mismatch");
 var stickRenderer=iceRoot.GetComponentsInChildren<UnityEngine.Renderer>(true).Single(r=>r.name=="Stick");
 var iceRenderer=iceRoot.GetComponentsInChildren<UnityEngine.Renderer>(true).Single(r=>r.name=="Ice");
 if(stickRenderer.sharedMaterial.name!="IcePopStick" || iceRenderer.sharedMaterial.name!="IcePop")throw new System.Exception("Material mismatch");
 result.Add("iceScale="+iceRoot.transform.localScale+";stick="+stickRenderer.sharedMaterial.name+";brown="+stickRenderer.sharedMaterial.GetColor("_Color")+";longScale="+root.longTestPlug.transform.localScale);
 iceRoot.transform.localScale=UnityEngine.Vector3.one;stickRenderer.sharedMaterial=iceRenderer.sharedMaterial;
 if(!LumaKroma.Sps2SetupAssistant.Editor.Generation.FullSetupGenerator.ShowTestPlug(descriptor,out message))throw new System.Exception(message);
 if(iceRoot.transform.localScale!=UnityEngine.Vector3.one*.7f || stickRenderer.sharedMaterial.name!="IcePopStick")throw new System.Exception("Reuse update failed");
 UnityEditor.Undo.FlushUndoRecordObjects();UnityEditor.Undo.PerformUndo();
 if(iceRoot.transform.localScale!=UnityEngine.Vector3.one || stickRenderer.sharedMaterial!=iceRenderer.sharedMaterial)throw new System.Exception("Undo failed");
 UnityEditor.Undo.PerformRedo();
 if(iceRoot.transform.localScale!=UnityEngine.Vector3.one*.7f || stickRenderer.sharedMaterial.name!="IcePopStick")throw new System.Exception("Redo failed");
 result.Add("reuseAndUndoRedo=PASS");
