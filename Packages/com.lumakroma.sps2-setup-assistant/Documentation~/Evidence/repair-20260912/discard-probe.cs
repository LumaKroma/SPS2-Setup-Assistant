...
settings=redone.settings.Copy(); settings.parts.Find(p=>p.id=="earLeft").included=false;
if(!LumaKroma.Sps2SetupAssistant.Editor.Generation.FullSetupGenerator.Apply(copyDescriptor,settings,false,out root,out message))return "discard fixture failed: "+message;
...
observations.Add("prefabDiscardRestore="+(restored.identity==redone.identity&&restored.sockets.Count==15&&restored.testPlug!=null&&contents.GetComponentsInChildren<LumaKroma.Sps2SetupAssistant.Sps2SetupRoot>(true).Length==0));
