...
foreach(bool regenerate in new[]{false,true}) {
 var go=UnityEngine.Object.Instantiate(original.gameObject);go.name="CurrentSetupProbe";
 ...
 try {
  ...
  bool assetLoaded=root.asset!=null;
  {var settings=root.settings.Copy();var before=root.sockets.Select(s=>s.pose.position).ToArray();string error;if(!LumaKroma.Sps2SetupAssistant.Editor.Generation.FullSetupGenerator.Apply(a,settings,regenerate,out root,out error))throw new System.Exception(error);if(!regenerate&&!before.SequenceEqual(root.sockets.Select(s=>s.pose.position)))throw new System.Exception("Nonreplacement moved poses");}
  ...
  results.Add(new{regenerate,assetLoaded,built,residue,sequence,errors});
 }finally{
