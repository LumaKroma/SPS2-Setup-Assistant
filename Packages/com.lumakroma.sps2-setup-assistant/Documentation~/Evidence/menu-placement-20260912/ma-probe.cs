...
try {
 ...
 LumaKroma.Sps2SetupAssistant.Editor.Model.FullSetupCatalog.ApplyPreset(settings,2); settings.localOnly=false; settings.modularAvatar=true;
 ...
 result.Add("localOnlyMenuAbsent="+!seen.SelectMany(m=>m.controls).Any(c=>c.name=="Local Only"||c.name.Contains("Stealth Mode"))); var lp=descriptor.expressionParameters.parameters.Single(p=>p.name.EndsWith("stealth")); result.Add("localOnlySaved="+lp.saved+";default="+lp.defaultValue);
